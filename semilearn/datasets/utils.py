...
def split_ssl_data(args, data, targets, num_classes,
    ...
    #    lb_idx = lb_index
    #    print('lb index worked')
    #if ulb_idx is not None:
    #    ulb_idx = ulb_index
    #    print('ulb index worked')
...
#Add utils function for network data augmentation. (Edits: Robin Bhoo)
def random_case_swap(dat: np.ndarray, p: float=0.5):
    """Given a payload_byte network traffic data with ASCII values, randomly alters the values for alphabet
    representations to swap the lower/upper case. 

    Args:
        dat (np.ndarray): raw network data with payload_byte values.
        p (float, optional): probability threshold for the transformation to take place. Defaults to 0.5.

    Returns:
        swapped (np.ndarray): modified network data.
    """
    assert len(dat) == 1504, 'not the right shape for a single payload_byte sample'
    swapped = dat.copy()
    # Check if the element is within the ASCII range for alphabets
    for i in range(len(swapped)): # Each sample
        if swapped[i] == 0: break # End loop when encountering padding [NULL]
        if 65 <= swapped[i] <= 122:
            # Check if the element corresponds to a lowercase letter
            if 97 <= swapped[i] <= 122:
                # Randomly swap the lowercase letter to uppercase based on probability p
                if random.random() < p: swapped[i] -= 32
            else:
                # Randomly swap the uppercase letter to lowercase based on probability p
                if random.random() < p: swapped[i] += 32
    return swapped


def random_mask(dat: np.ndarray, p:float=0.15):
    """Randomly masks out the inputs in the network traffic data by setting it to 0 ([NULL] in ASCII).

    Args:
        dat (np.ndarray): raw network data with payload_byte values.
        p (float, optional): probability threshold for the transformation to take place. Defaults to 0.15.

    Returns:
        masked (np.ndarray): masked network data.
    """
    assert len(dat) == 1504, 'not the right shape for a single payload_byte sample'
    masked = dat.copy()
    for i in range(len(masked)):
        if masked[i] == 0: break # End loop when encountering padding [NULL]
        elif random.random()< p: masked[i] = 0
